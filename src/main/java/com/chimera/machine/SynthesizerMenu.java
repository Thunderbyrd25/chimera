...
public class SynthesizerMenu extends AbstractContainerMenu {
    ...
    public SynthesizerMenu(int containerId, Inventory playerInventory, SynthesizerBlockEntity blockEntity, int upgradeSlotCount) {
        super(ChimeraMenus.SYNTHESIZER.get(), containerId);
        ...
        addSlot(new SlotItemHandler(blockEntity.getInventory(), SynthesizerBlockEntity.SLOT_INPUT, 44, 17));
        addSlot(new SlotItemHandler(blockEntity.getInventory(), SynthesizerBlockEntity.SLOT_OUTPUT, 116, 17));
    ...
    public boolean stillValid(Player player) {
        return stillValid(ContainerLevelAccess.create(blockEntity.getLevel(), blockEntity.getBlockPos()), player, ChimeraBlocks.SYNTHESIZER.get());
    }
    ...
    public ItemStack quickMoveStack(Player player, int index) {
        ...
        } else if (blockEntity.findSynthesisPool(slotStack).isPresent()) {
            if (!this.moveItemStackTo(slotStack, SynthesizerBlockEntity.SLOT_INPUT, SynthesizerBlockEntity.SLOT_INPUT + 1, false)) {
                return ItemStack.EMPTY;
            }
